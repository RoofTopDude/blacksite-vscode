...
<script nonce="${nonce}">(() => {
  ...
  function safePreview(preview) { const base = preview && preview.html && preview.html.trim() ? preview.html : '<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;min-height:100%;}</style></head><body></body></html>'; const code = preview && preview.code ? preview.code : ''; const script = '<script type="module" nonce="${nonce}">' + code + '<\\/script>'; const pos = base.search(/<\\/head\\s*>/i); return pos >= 0 ? base.slice(0, pos) + script + base.slice(pos) : base + script; }
