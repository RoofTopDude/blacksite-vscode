...
export function buildSystemPrompt(snapshot: WorkspaceSnapshot): string {
  ...
  parts.push(
    ...
    "- **Codebase Map:** map_link draws a persistent annotated relation between two files on the Map view when you find a meaningful non-import relationship worth showing spatially — event flows, IPC/message routes, config-to-consumer links. Keep the note to one short sentence; it renders directly on the map alongside imports and live trace activity. map_link_list checks what's already drawn (optionally filtered to one file) before adding more, and map_link_remove deletes a relation by id.",
