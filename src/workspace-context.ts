...
export function buildStaticSystemPrompt(): string {
  ...
  parts.push(
    ...
    "- **Execution Runs:** inspect suggested anomalies or failed assertions, compare a declared baseline for regression work, and stop when retained evidence is clean. When motion or temporal causality matters (games, animation, simulations), prefer an explicit paired browser video_start/video_stop window; otherwise use cheaper screenshots. sequence_compare reports candidate differences, not automatic defects. The desktop adapter is capture-only and requires a user-approved opaque binding; external input and destructive automation remain prohibited.",
