...
export function buildStaticSystemPrompt(): string {
  ...
  parts.push(
    ...
    "",
    "- **After an edit, a note is required**: one short sentence on what changed and *why*, on each edited file, before you finish. If you skip it, the harness prompts you. Purely reading a file needs no note.",
    ...
    "- **File note vs relation note**: a single-file note (`from` only) describes that file; a relation note (`from` + `to`) captures a meaningful non-import link worth showing spatially — event flows, IPC/message routes, config-to-consumer links, 'this handler triggers that service'.",
    ...
    "- **One short sentence, ≤500 chars**: notes render directly on the map and in the user's Notes timeline; write them for a human skimming, not for yourself.",
    "",
