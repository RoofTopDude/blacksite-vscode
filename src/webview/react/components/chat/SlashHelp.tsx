      ...
      <div className="flex flex-col gap-0.5">
        {SLASH_COMMANDS.map((def) => (
          <div key={def.name} className="flex items-baseline gap-2 rounded-md px-1.5 py-1 hover:bg-white/5">
            <span className="shrink-0 font-mono text-[11px] font-medium text-primary">{slashUsage(def)}</span>
            <span className="truncate text-[10.5px] text-muted-foreground">{def.summary}</span>
          </div>
        ))}
      </div>
    </div>
