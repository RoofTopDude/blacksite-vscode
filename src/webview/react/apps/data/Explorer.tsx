      ...
      <div className="overflow-y-auto p-3">
        {!s.selectedObject ? (
          ...
          <>
            {desc && (
              <div className="mb-2">
                <div className="text-lg font-semibold text-foreground">{desc.name} <span className="font-normal text-muted-foreground">{desc.type}</span></div>
                <div className="text-sm text-muted-foreground">{desc.rowCount} rows · {desc.columns.length} columns · {desc.indexes.length} index(es)</div>
                <div className="my-2 flex flex-wrap gap-1">
                  {desc.columns.map((c: any) => (
                    <span key={c.name} title={`${c.type}${c.notNull ? " NOT NULL" : ""}`} className={`rounded-full border px-2 py-px font-mono text-xs ${c.primaryKey ? "border-primary/50 text-primary" : "border-border bg-white/5 text-muted-foreground"}`}>{c.name}</span>
                  ))}
                </div>
