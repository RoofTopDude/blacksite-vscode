...
export function QueryTab() {
  ...
  return (
    <div className="flex flex-1 flex-col gap-2.5 overflow-y-auto p-3">
      <Input value={s.queryName} onChange={(e) => actions.setQueryName(e.target.value)} placeholder="Query name (for saving)" className="h-7 text-sm" />
      <Textarea
        value={s.sql}
        onChange={(e) => actions.setSql(e.target.value)}
        placeholder="SELECT * FROM v_recent_agent_activity LIMIT 50;"
        className="min-h-[120px] resize-y font-mono text-base leading-relaxed"
      />
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-sm text-muted-foreground">{s.queryStatus}</span>
      </div>
