  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <header className="shrink-0 border-b border-border px-3 py-2.5">
        <div className="text-[13px] font-semibold text-foreground">Base Context</div>
        <div className="mt-0.5 text-[10.5px] leading-snug text-muted-foreground">Static, cross-conversation project context. Enabled topics and their files load into every agent run.</div>
        ...
        <div className="mt-1.5 truncate font-mono text-[10px] text-muted-foreground">{activeFile ? `Active file: ${activeFile}` : "No active editor file."}</div>
      </header>

      <div className="flex-1 overflow-y-auto px-3 py-3">
        {doc.topics.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border bg-white/[0.02] p-4 text-[11px] leading-relaxed text-muted-foreground">
              ...
              <section key={topic.id} className="overflow-hidden rounded-xl border border-border bg-white/[0.03]">
                <div className="flex items-start justify-between gap-2 p-2.5 pb-2">
                  <div className="min-w-0 flex-1">
                    <input
                      ...
                      className="w-full rounded-md border border-transparent bg-transparent px-1.5 py-1 text-[13px] font-semibold text-foreground outline-none focus:border-primary/50 focus:bg-white/[0.04]"
                    />
                    <div className="mt-1 flex flex-wrap gap-1 px-1.5 text-[10px] text-muted-foreground">
                ...
                <div className="flex flex-col gap-2 px-2.5 pb-2.5">
                  <Textarea
                    ...
                    className="min-h-[96px] resize-y rounded-lg text-[12px] leading-relaxed"
                  />

                  <div className="flex flex-col gap-1.5">
                    {topic.files.length === 0 ? (
                      <div className="text-[10px] text-muted-foreground">No linked files yet.</div>
                      ...
                      <div key={file.id} className="flex items-center justify-between gap-2 rounded-md border border-border bg-white/[0.025] px-2 py-1.5">
                        <div className="flex min-w-0 items-center gap-1.5">
                          ...
                          <span className="truncate font-mono text-[10.5px] text-foreground" title={file.path}>{file.path}</span>
                        </div>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[9.5px] text-muted-foreground">Updated {topic.updatedAt || topic.createdAt || "recently"}</span>
